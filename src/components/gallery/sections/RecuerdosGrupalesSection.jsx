...
const QUOTES = [
  "Lo que se comparte, se multiplica en la memoria.",
  "Un grupo no es la suma de sus partes, sino el eco de lo vivido juntos.",
  "Los recuerdos grupales son el tesoro que nadie puede llevarse solo.",
];
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          >
            <GalleryImage
              src={current.viewUrl}
              alt={`Recuerdos Grupales — ${index + 1}`}
              className="w-full h-full"
            />
          </motion.div>
        </motion.div>
        ...
        {loopImages.map((img, i) => {
          ...
          return (
            ...
            >
              <GalleryImage
                src={img.viewUrl}
                alt={`Recuerdos Grupales — ${realIndex + 1}`}
                className="w-full h-full transition-transform duration-[1.2s] ease-out group-hover:scale-105"
              />
              <div
                className="absolute inset-0 transition-opacity duration-700"
...
export default function RecuerdosGrupalesSection() {
  ...
  useEffect(() => {
    base44.functions
      .invoke("listDriveImages", { folderPath: "Web/08 - Grupales" })
  ...
  return (
    <section id="grupales" className="py-24 md:py-32 px-8 md:px-16">
      <SectionHeader
        number="05"
        title="Recuerdos Grupales"
        description="Los instantes compartidos. El viaje que se vuelve memoria colectiva."
      />
      ...
      {loading ? (
        ...
        >
          ...
          >
            <Users size={18} strokeWidth={1.5} style={{ color: "#8C867A" }} />
            <p
